...
export const getContacts = api({ expose: true, method: "GET", path: "/api/v1/contacts" }, async (req) => {
    try {
        ...
        LIMIT $2 OFFSET $3
      `;
        const rows = await crmDB.rawQueryAll(query, companyId, limit, offset);
        // Count total
        const countResult = await crmDB.rawQueryRow(`SELECT COUNT(*) as total 
         FROM people p
         LEFT JOIN companies c ON p.company_id = c.id
         WHERE c.id IN (SELECT id FROM companies WHERE id = $1)`, companyId);
...
//# sourceMappingURL=get_contacts.js.map