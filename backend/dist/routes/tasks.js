...
router.get('/', async (req, res) => {
    try {
        const { assignedTo, personId, dealId, isCompleted, dueBefore, dueAfter, limit = 100, offset = 0 } = req.query;
...
router.post('/', async (req, res) => {
    try {
        const { companyId = 1, personId, dealId, assignedTo, title, description, dueDate, priority = 'medium' } = req.body;
        if (!title || !assignedTo) {
