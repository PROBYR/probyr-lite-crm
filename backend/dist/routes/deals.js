...
router.get('/', async (req, res) => {
    try {
        const { stageId, personId, limit = 100, offset = 0 } = req.query;
...
router.get('/table', async (req, res) => {
    try {
        const { pipelineId, sortBy = 'created_at', sortOrder = 'DESC', limit = 100, offset = 0 } = req.query;
        let whereClause = 'WHERE 1=1';
        const queryParams = [];
        let paramIndex = 1;
        if (pipelineId) {
...
router.post('/', async (req, res) => {
    ...
    try {
        ...
        const { companyId, personId, stageId, title, value = 0, expectedCloseDate, probability = 0, notes, assignedTo } = req.body;
        if (!title || !stageId) {
