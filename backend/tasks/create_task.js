...
// Creates a new task.
export const createTask = api({ expose: true, method: "POST", path: "/tasks" }, async (req) => {
    const taskRow = await crmDB.queryRow `
    ...
    const query = `
      ...
      WHERE t.id = $1
    `;
    const row = await crmDB.rawQueryRow(query, taskRow.id);
    if (!row) {
