...
export const updatePerson = api<UpdatePersonParams & UpdatePersonRequest, Person>(
  { expose: true, method: "PUT", path: "/people/:id" },
  async (req) => {
    const tx = await crmDB.begin();
    
    try {
      // Check if person exists
      const existingPerson = await tx.queryRow<{ id: number }>`
