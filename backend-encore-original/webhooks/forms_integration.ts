...
export const processFormSubmission = api<FormSubmissionRequest, FormSubmissionResponse>(
  { expose: true, method: "POST", path: "/webhooks/form-submission" },
  async (req) => {
    ...
    try {
      ...
      let person = await tx.queryRow<{ id: number }>`
      ...
      if (person) {
        ...
        // Create new person
        const newPerson = await tx.queryRow<{ id: number }>`
      ...
      const firstStage = await tx.queryRow<{ id: number }>`
      ...
      const deal = await tx.queryRow<{ id: number }>`
